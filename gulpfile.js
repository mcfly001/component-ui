...
var buildPackage = function(packagename){
  gulp.task(`build:${packagename}`, gulp.series(`del:${packagename}`, 'dealcss', function(done){
    ...
    let output = {
      path: path.resolve(__dirname, `./publish/component-ui/lib/${packagename}`),
      filename: 'index.js',
      library: packagename,
      libraryTarget: 'umd',
      umdNamedDefine: true
...
gulp.task('dealcss', function(){
  return gulp.src(__dirname + '/src/index.scss')
    ...
    .pipe(gulp.dest(__dirname + '/publish/component-ui/lib'))
})
...
gulp.task('build', gulp.series('del', 'dealcss', function(done){
  ...
  dirs.forEach(function(item){
    if(item !== 'components' && item !== 'scss' && item.indexOf('.') < 0){
      entry[item] = path.join(__dirname, `./packages/${item}/index.js`)
    }
  })

  let output = {
    path: path.resolve(__dirname, './publish/component-ui/lib'),
